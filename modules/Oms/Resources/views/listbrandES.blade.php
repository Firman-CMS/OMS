            ...
            <div class="box">
               ...
               @foreach($brandResult as $brandResultDetail)
                <tr>
                  ...
                  <td>
                      <button class="btn btn-mini" onclick="pick('{{$brandResultDetail}}');">Pick</button>
                  </td>
                </tr>
                @endforeach
                </tbody>
                ...
                <div class="row">
                    <div class="col-xs-12 col-md-12 pull-right">
                        Page :  
                        @if($brandPage>1)
                        <a href="{{route('oms.listbrandES').'/'.($brandPage-1)}}"><button class="btn-default" id="previous"> << </button></a>
                        @endif
                        <button class="btn-default" disabled="disabled"> {{$brandPage}} </button>
                          
                        @if($brandTotal>$brandPage)
                        <a href="{{route('oms.listbrandES').'/'.($brandPage+1)}}"><button class="btn-default" id="next"> >> </button></a>
                        @endif
                        of {{ $brandTotal }}                    
                    </div>
                </div>
...
//            $("#list").DataTable();
            ...
            {
            window.opener.document.form.brand.value = value1;
                    window.close();
            }
</script>    
@stop